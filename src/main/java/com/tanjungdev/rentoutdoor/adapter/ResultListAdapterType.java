...
public class ResultListAdapterType extends ArrayAdapter<Category> implements View.OnClickListener {
        //Sudah Konek dengan getCarRent();
        private final LayoutInflater inflater;
        private final ArrayList<Item> searchResultList;
        private Activity activity;
        private ResultListFragment.ResultListCallbacks mCallbacks;

        public ResultListAdapterType(Activity activity, ResultListFragment.ResultListCallbacks mCallbacks, ArrayList<Item> searchResultList) {
            super(activity, R.layout.layout_result_list_type);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.layout_result_list_type, null);
            row = new ViewHolder();
        ...
        row.itemTitleTV.setText(item.getCarTitle());
        row.itemAddressTV.setText(trimAdress2);
        row.itemCarName.setText(item.getTitle());
        if (item.isVerified()) {
    ...
    /*
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                convertView = inflater.inflate(R.layout.layout_result_list_rental, null);
                row = new ViewHolder();
                row.parentView = (RelativeLayout) convertView.findViewById(R.id.parentView);
                row.itemIconImgView = (ImageView) convertView.findViewById(R.id.itemIconImgView);
                row.itemTitleTV = (TextView) convertView.findViewById(R.id.itemTitleTV);
            ...
            row.txtPrice.setText(item.getCarPrice());
            if (item.isVerified()) {
                row.verificationImgView.setVisibility(View.VISIBLE);
            } else {
                row.verificationImgView.setVisibility(View.INVISIBLE);
